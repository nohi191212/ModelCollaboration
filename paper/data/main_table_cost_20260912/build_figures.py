...
curves=[]
selected=[]
for g,tag in zip(groups,['minicpm','qwen']):
    expert=g['key'][1]
...
for field,stem in [('estimated_gflops_per_input','flops_performance'),('estimated_time_ms_per_input','latency_performance')]:
    ...
    for ax,key,title in zip(axes,keys,['gRefCOCO / M','Construction / Q']):
        panel(ax,key,field)
...
for col,key in enumerate(keys):
    ...
    axes[0,col].text(.5,1.14,'gRefCOCO / M' if col==0 else 'Construction / Q',transform=axes[0,col].transAxes,ha='center',fontsize=7)
