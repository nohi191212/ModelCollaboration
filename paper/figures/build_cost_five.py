...
plt.rcParams.update({'font.family':'serif','font.serif':['Times New Roman'],'font.size':8,'axes.spines.top':False,'axes.spines.right':False,'axes.linewidth':.6,'pdf.fonttype':42,'mathtext.fontset':'stix'})
fig,axes=plt.subplots(2,2,figsize=(3.44,2.05))
fig.subplots_adjust(left=.16,right=.985,top=.82,bottom=.175,hspace=.72,wspace=.46)
records=[];selected=[]
for col,(key,c) in enumerate(components.items()):
 g=next(g for g in groups if '|'.join(g['key'])==key)
 for method,color,label in [('learned','#176f91','LR'),('confidence','#c78324','conf.')]:
  ...
  for row,unit in enumerate(['gflops','ms']):
   ...
   ax.yaxis.set_major_locator(MaxNLocator(3));ax.tick_params(labelsize=7.5,pad=1.5);ax.grid(axis='y',color='#dce1e4',lw=.4)
   if row==0:
    ax.set_xscale('log');ax.set_xlabel('GFLOPs / input',fontsize=8,labelpad=1);ax.xaxis.set_major_locator(LogLocator(base=10,numticks=4));ax.xaxis.set_minor_formatter(NullFormatter())
   else:ax.set_xlabel('Est. seconds / input',fontsize=8,labelpad=1);ax.xaxis.set_major_locator(MaxNLocator(3))
   if col==0:ax.set_ylabel('Test metric (%)',fontsize=8,labelpad=2)
 ...
 axes[0,col].set_title(['InstanceVG / M','YOLO26X / Q'][col],fontsize=8.5,pad=3)
handles,labels=axes[0,0].get_legend_handles_labels();fig.legend(handles,labels,loc='upper center',bbox_to_anchor=(.56,1.015),ncol=2,frameon=False,fontsize=8,handlelength=1.8)
