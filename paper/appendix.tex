...
\input{sections/supplement_data}
\input{sections/supp_examples}
...
\input{tables/main_full}
\input{tables/thresholds}
\input{tables/outcomes_full}
\begin{figure}[p]\centering\includegraphics[width=\textwidth]{figures/budget_all_m.pdf}\caption{LR and confidence results for MiniCPM. Horizontal axes show the nominal validation budget; panels report test performance and transferred test call rates, including both endpoints.}\end{figure}
\begin{figure}[p]\centering\includegraphics[width=\textwidth]{figures/budget_all_q.pdf}\caption{LR and confidence results for Qwen. Same conventions.}\end{figure}
\clearpage
\begin{figure}[p]\centering\includegraphics[width=\textwidth]{figures/full_budget_reselection.pdf}\caption{Old checkpoints with normalization versus full-budget checkpoint/normalization selection at identical actual calls. Five-seed means and standard-deviation bands. This isolates the incremental reselection effect after changing the score.}\end{figure}
\clearpage
...
Let $c_S,c_L$ denote correctness of the fixed specialist and VLM answers. The three-state target distinguishes rescue, harm and an unchanged result, merging both-correct and both-wrong examples. It uses cross-entropy and ranks by predicted rescue probability minus harm probability. Gain regression uses squared error with target $c_L-c_S$ and ranks by its scalar output. For construction, targets are averaged over the four event labels. The four-state rows change only the score computed from the same trained heads; they do not represent four separate training runs.
...
For $\mathrm{LR}_{PH}$, the scalar loss is $e_S\log(1+e^{-r})+e_L\log(1+e^r)$, where $e_S,e_L$ are observed errors, averaged over construction events. This is the zero-added-cost post-hoc comparison. Its unmodified scalar score is swept for budget curves; this differs from reporting only the native $r>0$ decision. The VLM and specialist answers remain fixed. Other archived cost coefficients are not selected using test results.

The four-state score $r_{4S}=\log[(p_R+p_B)/(p_H+p_B)]$ estimates the conditional minimizer of the zero-cost post-hoc logistic loss when both conditional error rates are positive. This reuses the error-ratio row, not a new training run. The separately trained scalar $r$ need not equal $r_{4S}$ for finite fitted models. Avg. is the arithmetic mean of the four task columns (accuracy for three tasks and macro-F1 for construction), not a pooled accuracy or a significance test.
\begin{table}[H]\input{tables/objective_main}\end{table}
\begin{figure}[p]\centering\includegraphics[width=\textwidth]{figures/score_controls.pdf}\caption{Earlier experiments: changing four-state scoring while holding weights fixed, with separately trained zero-cost post-hoc deferral. Both use the archived checkpoint-selection range. These are equal-call curves, not test-selected best-threshold envelopes.}\end{figure}
\clearpage
