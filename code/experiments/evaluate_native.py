...
import numpy as np
import torch
from k_class_losses import labels_from_records, native_scores, native_records
...
for split in ['val','test']:
    ...
    if model is None:
        ...
        k=200 if cfg['task']=='cub' else 2
        model.head[-1]=torch.nn.Linear(128,k+1).to(device)
...
for split,raw in logits.items():
    scores[split]={cfg['ranking']:native_scores(raw,cfg['native_method'])}
    records[split]=native_records(records[split],raw,cfg['task'])
...
result['native_zero_threshold']={split:{'metric':formal.metric(records[split],cfg['endpoint'],scores[split][cfg['ranking']]>0),'calls':float((scores[split][cfg['ranking']]>0).mean())} for split in ['val','test']}
result['retained_answer']='K-class branch argmax, NOT old frozen specialist prediction'
