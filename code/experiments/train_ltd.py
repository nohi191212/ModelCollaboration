...
from ltd_methods import loss_values, scores, output_count
...
def main():
    ...
    if cfg['ltd_method']!='four_state':model.head[-1]=torch.nn.Linear(128,output_count(cfg['ltd_method'])).to(device)
    ...
    y=torch.tensor(states_np,device=device,dtype=torch.float32)
    ...
    for epoch in range(1,cfg['max_epochs']+1):
        ...
        for b in range(0,len(y),cfg['batch_size']):
            ...
            loss=loss_values(raw,y[ids],cfg['ltd_method'],cfg['call_cost'])
        ...
        routing_scores=scores(raw,cfg['ltd_method'])
        metrics=ranked_curve(records['val'],cfg['endpoint'],routing_scores)
        ...
        if improved:
            ...
            np.save(dest/'validation_scores.npy',routing_scores)
