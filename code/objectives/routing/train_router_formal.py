...
def load_data(cfg, split, device):
    ...
    cached = ROOT / 'training_inputs' / expert
    if (cached / (split + '.npz')).is_file():
        metadata = next(row for row in json.loads((ROOT / 'input_manifest.json').read_text()) if row['expert'] == expert and row['split'] == split)
        if cfg['student'] == '8M' and cfg.get('hidden_layer') == metadata['hidden_layer'] and not cfg.get('hidden_layers'):
            rows = read_rows(cached / (split + '_records.jsonl'))
            with np.load(cached / (split + '.npz')) as source:
                data = {k: torch.from_numpy(source[k].copy()).to(device=device) for k in source.files}
