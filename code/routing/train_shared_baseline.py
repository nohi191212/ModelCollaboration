...
def main():
    ...
    ap.add_argument('--allow-input-ablation',action='store_true')
    a=ap.parse_args()
    ...
    if not a.allow_input_ablation and not {'image','hidden'}<=set(cfg['branches']):raise ValueError('Mandatory image and hidden branches missing')
    if not a.allow_input_ablation and cfg['task'] in ['nlvr2','grefcoco'] and 'text' not in cfg['branches']:raise ValueError('Real task text must be encoded')
    ...
    for key in [k for k in ['hidden','confidence'] if k in data['train']]:
    ...
    model=FeatureRouter(cfg,*(data['train'][k].shape[1] if k in data['train'] else 0 for k in ['hidden','confidence','output'])).to(device)
    ...
    for epoch in range(1,cfg['max_epochs']+1):
        ...
        for b in range(0,len(y),cfg['batch_size']):
            ...
            if cfg['target']=='error':loss=F.binary_cross_entropy_with_logits(raw,y[ids],reduction='none').mean(1)
