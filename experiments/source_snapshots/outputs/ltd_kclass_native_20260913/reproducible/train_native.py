...
from k_class_losses import labels_from_records, k_class_loss, native_scores, native_records
...
def main():
    ...
    k,class_labels,expert_correct=labels_from_records(records['train'],cfg['task'],cfg['endpoint'])
    model.head[-1]=torch.nn.Linear(128,k+1).to(device)
    expert_correct=expert_correct.to(device)
    ...
    for epoch in range(1,cfg['max_epochs']+1):
        ...
        for b in range(0,len(y),cfg['batch_size']):
            ...
            loss=k_class_loss(raw,y[ids],expert_correct[ids],cfg['native_method'])
        ...
        scores=native_scores(raw,cfg['native_method'])
        records['val']=native_records(records['val'],raw,cfg['task'])
