...
def main() -> None:
    ...
    prompt_text = (
        ...
        "For each of the four fixed rule slots, report whether that rule is violated, with a short reason and one or more normalized xyxy boxes using integer coordinates from 0 to 1000. "
    ...
    with args.output.open("w", encoding="utf-8") as destination:
        for index, row in enumerate(rows, 1):
            ...
            try:
                ...
                destination.flush()
                raise ValueError(f"sample {row['sample_id']} returned invalid output: {answer!r}") from error
            output["generalist_output"] = model_output
            destination.write(json.dumps(output, ensure_ascii=False) + "\n")
            destination.flush()
