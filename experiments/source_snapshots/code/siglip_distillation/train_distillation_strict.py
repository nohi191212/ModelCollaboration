"""Fresh strict-split distillation, optionally one GPU per student with a shared cache."""
...
def worker(worker_id,specs,shared,args,barrier,stores):
    ...
    experiment,rank=divmod(worker_id,world)
    gpu=experiment if args.one_gpu_per_experiment else rank
    size=specs[experiment];torch.cuda.set_device(gpu);torch.set_num_threads(2)
    dist.init_process_group('nccl',init_method='file://'+stores[experiment],rank=rank,world_size=world,device_id=torch.device(f'cuda:{gpu}'),timeout=timedelta(minutes=5))
    ...
    config=dict(model=model.config,parameters=counts,**vars(args),gpus=[gpu] if args.one_gpu_per_experiment else list(range(world)),world_size=world,size_millions=size)
    ...
    if rank==0:
        ...
        shutil.copyfile(Path(args.settings_file),dest/'exp_settings.md')
        for name in ['student_tokenizer.json','ready.json']:
            shutil.copyfile(Path(args.cache_dir)/name,dest/name)
    model.train()
    ...
    with (dest/'loss_steps.csv' if rank==0 else Path(os.devnull)).open('a' if args.resume else 'w',buffering=1,newline='') as log, (dest/'loss_epochs.csv' if rank==0 else Path(os.devnull)).open('a' if args.resume else 'w',buffering=1,newline='') as epochlog:
        ...
        if rank==0:
            (dest/'training_started.json').write_text(json.dumps({'unix_time':time.time(),'gpu':gpu,'epoch_start':begin+1,'steps_per_epoch':steps}))
        for epoch in range(begin,epochs):
            ...
            for step in range(steps):
                ...
                start=step*args.batch_size+b*rank//world;end=step*args.batch_size+b*(rank+1)//world
                ...
                if rank==0 and (step%20==0 or args.smoke_steps): print('LOSS',size,epoch+1,step+1,values.tolist(),'grad',gradnorm.item(),flush=True)
            ...
            if rank==0 and ((epoch+1)%25==0 or epoch+1==epochs):
        ...
        if args.smoke_steps:
            ...
            if rank==0: print('GPU REPLICAS IDENTICAL',size,world,flush=True)
    if rank==0:
        (dest/'completion.json').write_text(json.dumps({'status':'complete','epochs':epochs,'unix_time':time.time()}))
...
if __name__=='__main__':
    ...
    ap.add_argument('--one-gpu-per-experiment',action='store_true')
    ...
    devices=len(specs) if args.one_gpu_per_experiment else args.gpus_per_experiment
    assert not args.one_gpu_per_experiment or args.gpus_per_experiment==1
    assert torch.cuda.device_count()>=devices
    cache=Path(args.cache_dir)
    audit=json.loads((cache/'split_audit.json').read_text())
    assert audit['status']=='split_checks_passed'
    assert audit['heldout_image_identity_overlap']==audit['heldout_normalized_text_overlap']==0
    if args.resume: raise ValueError('This strict rerun must start fresh; old student resume is forbidden')
    assert (cache/'teacher_ready.json').is_file()
    torch.set_num_threads(8);torch.cuda.set_device(0)
    for gpu in range(1,devices):
    ...
    assert len(shared['images'])==len(shared['image_targets'])==audit['images']
    assert len(shared['text_ids'])==len(shared['text_targets'])==audit['texts']
